...
(function () {
    ...
    function render(data) {
        ...
        const dotsHtml = data.uptime_dots.map((d, i) => {
            ...
            const tip = date.toLocaleDateString() + ': ' + d;
            let cls = d;
            return `<div class="dot ${cls}" data-tip="${tip}"></div>`;
        }).join('');
    ...
    function renderCharts(data) {
        ...
        const labels = history.map(h => {
            ...
            return currentHours <= 6 ? d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        ...
        const chartOpts = {
            ...
            plugins: {
                legend: { position: 'top', labels: { color: '#8B949E', font: { size: 11 }, boxWidth: 12, padding: 16 } },
                tooltip: { backgroundColor: '#111517', titleColor: '#F2FAFF', bodyColor: '#ccc', borderColor: '#32434D', borderWidth: 1 },
            },
            scales: {
                x: { ticks: { color: '#8B949E', font: { size: 10 }, maxTicksLimit: 12 }, grid: { color: 'rgba(255,255,255,.04)' } },
                y: { ticks: { color: '#8B949E', font: { size: 10 } }, grid: { color: 'rgba(255,255,255,.04)' }, min: 0 },
            },
        };
        ...
        resourceChart = new Chart(ctx1, {
            ...
            data: {
                labels,
                datasets: [
                    { label: 'CPU %', data: history.map(h => h.cpu), borderColor: getComputedStyle(document.documentElement).getPropertyValue('--bar-cpu').trim(), backgroundColor: 'transparent', tension: 0.3, pointRadius: 0, borderWidth: 2 },
                    { label: 'RAM %', data: history.map(h => h.ram), borderColor: getComputedStyle(document.documentElement).getPropertyValue('--bar-ram').trim(), backgroundColor: 'transparent', tension: 0.3, pointRadius: 0, borderWidth: 2 },
                    { label: 'Disk %', data: history.map(h => h.disk), borderColor: getComputedStyle(document.documentElement).getPropertyValue('--bar-disk').trim(), backgroundColor: 'transparent', tension: 0.3, pointRadius: 0, borderWidth: 2 },
                ],
            },
        ...
        networkChart = new Chart(ctx2, {
            ...
            data: {
                labels,
                datasets: [
                    { label: 'In (Mbps)', data: history.map(h => h.net_in), borderColor: getComputedStyle(document.documentElement).getPropertyValue('--bar-net-in').trim(), backgroundColor: 'rgba(199,255,120,.08)', fill: true, tension: 0.3, pointRadius: 0, borderWidth: 2 },
                    { label: 'Out (Mbps)', data: history.map(h => h.net_out), borderColor: '#FFB778', backgroundColor: 'rgba(255,183,120,.08)', fill: true, tension: 0.3, pointRadius: 0, borderWidth: 2 },
                ],
            },
            options: chartOpts,
        });
